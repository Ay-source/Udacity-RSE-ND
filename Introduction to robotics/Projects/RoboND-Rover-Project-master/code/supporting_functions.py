...
def convert_to_float(string_to_convert):
      if ',' in string_to_convert:
            float_value = np.array(string_to_convert.replace(',','.'), dtype=np.float32)
      else: 
            float_value = np.array(string_to_convert, dtype=np.float32)
      return float_value
...
def update_rover(Rover, data):
      ...
      if Rover.start_time == None:
            ...
            Rover.samples_to_find = np.array(data["sample_count"], dtype=np.int32)
      ...
      # Near sample flag
      Rover.near_sample = np.array(data["near_sample"], dtype=np.int32)
      # Picking up flag
      Rover.picking_up = np.array(data["picking_up"], dtype=np.int32)
      # Update number of rocks collected
      Rover.samples_collected = Rover.samples_to_find - np.array(data["sample_count"], dtype=np.int32)
...
def create_output_images(Rover):
      ...
      tot_nav_pix = np.array(len((plotmap[:,:,2].nonzero()[0])), dtype=np.float32)
      # Next figure out how many of those correspond to ground truth pixels
      good_nav_pix = np.array(len(((plotmap[:,:,2] > 0) & (Rover.ground_truth[:,:,1] > 0)).nonzero()[0]), dtype=np.float32)
      # Next find how many do not correspond to ground truth pixels
      bad_nav_pix = np.array(len(((plotmap[:,:,2] > 0) & (Rover.ground_truth[:,:,1] == 0)).nonzero()[0]), dtype=np.float32)
      # Grab the total number of map pixels
      tot_map_pix = np.array(len((Rover.ground_truth[:,:,1].nonzero()[0])), dtype=np.float32)
